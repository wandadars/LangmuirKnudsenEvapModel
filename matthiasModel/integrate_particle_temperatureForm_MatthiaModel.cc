...
int main(void)
{
  ...
  double integrateMassEnergy(
    ...
    const int ITMAX=100 ;
    int iter = 0 ;
      cout<<"f2: "<<f2<<"\tNu: "<<Nu<<"\tCp_g: "<<cp_g<<"\tPr: "<<Pr_g<<"\ttau_p: "<<tau_p<<"\tC_liquid: "<<p.get_c()<<"\tTerm1Coeff: "<<fudge*f2*Nu*cp_g/(3.*Pr_g*tau_p*p.get_c())<<"\tTerm2Coeff: "<< hvap*mdot/(max(p.mass[0],1e-30f)*p.get_c())<<"\tBeta: "<<beta_b<<"\tMass "<<p.mass[0]<<"\tMdot "<<p.mdot<<endl;
